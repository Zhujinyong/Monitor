...
namespace Centa.Monitor.ApplicationService.Monitor
{
    ...
    {
        private readonly IRepository<HttpLogModel> _repository;

        private IUnitOfWork _unitOfWork;

        public HttpLogService(IRepository<HttpLogModel> repository, IUnitOfWork unitOfWork)
        {
        ...
        public PageDataView<HttpLogListDto> GetList(HttpLogSearchDto dto)
        {
            PageDataView<HttpLogListDto> result = new PageDataView<HttpLogListDto>() { };
            StringBuilder select = new StringBuilder();
            StringBuilder where = new StringBuilder();
            DynamicParameters parameters = new DynamicParameters();
            select.Append("SELECT B.* ,P.ProjectName FROM HttpLog AS B JOIN Project AS P ON B.webMonitorId=P.KeyId  ");
            ...
            result = _unitOfWork.GetPageData<HttpLogListDto>(select.ToString() + where.ToString(), ordery, dto.PageIndex, dto.PageSize, parameters);
            return result;
        }
