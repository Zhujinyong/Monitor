...
namespace Centa.Monitor.WebApi.Token
{
    public class TokenVerify : ITokenVerify
    {
        ...
        /// <returns></returns>
        public string WriteToken(UserModel account)
        {
            var claims = new[] {
                new Claim(JwtRegisteredClaimNames.NameId, account.DomainAccount)
            };
        ...
        public TokenModel ReadToken(string token)
        {
            try
            {
                ...
                {
                    DomainAccount = jwtToken.Claims.FirstOrDefault(p => p.Type == JwtRegisteredClaimNames.NameId).Value,
