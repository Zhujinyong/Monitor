    ...
    public class RedisHelper
    {
        ...
        static RedisHelper()
        {
            try
            {
                ...
                ConnectionString = string.Join(",", redisips);
                _connMultiplexer = ConnectionMultiplexer.Connect(ConnectionString);
                DefaultKey = "SignIn";
                AddRegisterEvent();
            }
        ...
        public bool StringSetJosn<T>(string redisKey, T redisValue, TimeSpan? expiry = null)
        {
            redisKey = AddKeyPrefix(redisKey);
            var json = Serializejosn(redisValue);
            return _db.StringSet(redisKey, json, expiry);
        }
        ...
        public T StringGetJosn<T>(string redisKey, TimeSpan? expiry = null)
        {
            redisKey = AddKeyPrefix(redisKey);
        ...
        public bool HashSetJosn<T>(string redisKey, string hashField, T redisValue)
        {
            redisKey = AddKeyPrefix(redisKey);
        ...
        /// <param name="hashField"></param>
        /// <returns></returns>
        public T HashGetJosn<T>(string redisKey, string hashField)
        {
            redisKey = AddKeyPrefix(redisKey);
            return DeserializeObject<T>(_db.HashGet(redisKey, hashField));
        ...
        private static string Serializejosn(object obj)
        {
            if (obj == null)
                return null;
