...
namespace Centa.Monitor.Common
{
    public class MemoryCacheHelper
    {
        ...
        /// <param name="seconds">缓存时长(默认:1小时)</param>
        /// <param name="isSliding">是否滑动过期</param>
        /// <returns></returns>
        public static bool Set(string key, object value, int? seconds = 3600, bool isSliding = false)
        {
            if (string.IsNullOrEmpty(key))
            {
                throw new ArgumentNullException(nameof(key));
            ...
            TimeSpan timeSpan = TimeSpan.FromSeconds(Convert.ToDouble(seconds.Value));
            MemoryCacheEntryOptions option = new MemoryCacheEntryOptions();
            _memoryCache.Set(key, value, isSliding ? option.SetSlidingExpiration(timeSpan) : option.SetAbsoluteExpiration(timeSpan));
            return Exists(key);
        }
