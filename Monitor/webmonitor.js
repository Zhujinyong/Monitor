...
(function (window) {
  ...
  var
    ...
    WEB_MONITOR_ID = localStorage.CUSTOMER_WEB_MONITOR_ID 

    ,WEB_USER_ID = localStorage.CUSTOMER_WEB_USER_ID 

    // 是否打印日志
    , IS_SHOW_CONSOLE_LOG = localStorage.IS_SHOW_CONSOLE_LOG || false
  ...
  function MonitorBaseInfo() {
    this.handleLogInfo = function (type, logInfo) {
      ...
      switch (type) {
        case ELE_BEHAVIOR:
        if(logInfo.behaviorType=='click'&&logInfo.innerText==''&&logInfo.inputValue==''){
          return;
        }
  ...
  function setCommonProperty() {
    this.happenTime = utils.dateFormat(new Date(),'yyyy-MM-dd hh:mm:ss'); // 日志发生时间
  ...
  function HttpLogInfo(uploadType, simpleUrl, url, status, statusText, statusResult, responseText, currentTime, loadTime) {
    ...
    this.happenTime = utils.dateFormat(new Date(currentTime),'yyyy-MM-dd hh:mm:ss');  // 客户端发送时间
  ...
  function performanceGetEntries() {
    ...
    if (window.performance && typeof window.performance.getEntries === "function") {
      ...
      for (var m = 0; m < scripts.length; m ++) {
        var exist=errorScripts.some(function(v){
          return v==scripts[m];
        });
        var resourceLoadInfo = new ResourceLoadInfo(RESOURCE_LOAD, scripts[m], "script", exist?"0":"1");
        resourceLoadInfo.handleLogInfo(RESOURCE_LOAD, resourceLoadInfo);
      }
      for (var m = 0; m < links.length; m ++) {
        var exist=errorLinks.some(function(v){
          return v==links[m];
        });
        var resourceLoadInfo = new ResourceLoadInfo(RESOURCE_LOAD, links[m], "link", exist?"0":"1");
        resourceLoadInfo.handleLogInfo(links, resourceLoadInfo);
      }
    }
  }
  ...
  function recordHttpLog() {
    ...
    function handleHttpResult(i, tempResponseText) {
      ...
      //var currentTime = utils.dateFormat(new Date(),'yyyy-MM-dd hh:mm:ss');
  ...
  function MonitorUtils() {
    ...
    this.screenShot = function (cntElem, description) {
      ...
      var canvas = document.createElement("div"); //创建一个canvas节点
      ...
      window.html2canvas && window.html2canvas(cntElem,opts).then(function(canvas) {
       ...
       // var tempCompress = dataURL.replace("data:image/png;base64,", "");
        ...
        var compressedDataURL = (dataURL.replace("data:image/jpeg;base64,", ""));
        var screenShotInfo = new ScreenShotInfo(SCREEN_SHOT, description, compressedDataURL)
        screenShotInfo.handleLogInfo(SCREEN_SHOT, screenShotInfo);
      });
    }
  ...
  window.monitor = {
    ...
    wm_upload: function (url, type, index, description) {
      ...
      var logParams = {
        ...
        happenTime: utils.dateFormat(new Date(),'yyyy-MM-dd hh:mm:ss'),
     ...
     * @param userTpye
     */
    wm_init_user: function (userId, projectID) {
      if (!userId) console.warn('userId 初始化值为0(不推荐) 或者 未初始化');
      if (!projectID) console.warn('projectID 初始化值为0(不推荐) 或者 未初始化');
      WEB_USER_ID = userId ;
      WEB_MONITOR_ID = projectID ;
      return 1;
    },
    /**
     * 元素截图
     ...
     */
    wm_element_shot: function (element,description) {
      setTimeout(function () {
        utils.screenShot(element, description)
      }, 500);
    },
    /**
